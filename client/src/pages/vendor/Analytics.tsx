    ...
    return (
        <div className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6">
            ...
            <Card className="mb-4 sm:mb-6">
                <CardHeader>
                    <CardTitle className="text-lg sm:text-xl">Last 7 Days Performance</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                        <div>
                            <p className="text-xs sm:text-sm text-muted-foreground mb-1">Total Orders</p>
                            <p className="text-xl sm:text-2xl font-bold text-primary">{analytics.weekStats.totalOrders}</p>
                        </div>
                        <div>
                            <p className="text-xs sm:text-sm text-muted-foreground mb-1">Total Revenue</p>
                            <p className="text-xl sm:text-2xl font-bold text-semantic-success">₹{analytics.weekStats.totalRevenue.toFixed(2)}</p>
                        </div>
                    </div>
                    <div className="space-y-2">
                        {analytics.weekStats.dailyBreakdown.map((day) => (
                            <div key={day.date} className="flex items-center justify-between p-2 sm:p-3 bg-muted/50 rounded">
                                <span className="font-medium text-xs sm:text-sm">{new Date(day.date).toLocaleDateString()}</span>
                                <div className="flex items-center gap-2 sm:gap-4">
                                    <Badge variant="secondary" className="text-xs sm:text-sm">{day.orders} orders</Badge>
                                    <span className="font-bold text-semantic-success text-xs sm:text-sm">₹{day.revenue.toFixed(2)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-lg sm:text-xl">Top Selling Products (Last 30 Days)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3">
                            {analytics.topProducts.length === 0 ? (
                                <p className="text-muted-foreground text-center py-6 sm:py-8 text-sm sm:text-base">No data available</p>
                            ) : (
                                analytics.topProducts.map((product, index) => (
                                    <div key={product.productId} className="flex items-center justify-between p-2 sm:p-3 bg-muted/50 rounded">
                                        <div className="flex items-center space-x-2 sm:space-x-3">
                                            <Badge variant="default" className="w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm p-0">
                                                {index + 1}
                                            </Badge>
                                            <div>
                                        ...
                                        <span className="font-bold text-semantic-success text-xs sm:text-sm">₹{product.revenue.toFixed(2)}</span>
                                    </div>
                                ))
                            )}
                        </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-lg sm:text-xl">Revenue by Category (Last 30 Days)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3">
                            {analytics.revenueByCategory.length === 0 ? (
                                ...
                                analytics.revenueByCategory.map((category) => (
                                    <div key={category.category} className="p-2 sm:p-3 bg-muted/50 rounded">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="font-medium text-xs sm:text-sm">{category.category}</span>
                                            <span className="font-bold text-semantic-success text-xs sm:text-sm">₹{category.revenue.toFixed(2)}</span>
                                        </div>
